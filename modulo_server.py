...
def add_node(node_id):
    ...
    if node_id not in ep2017.nodes:
        ep2017.nodes.append(node_id)
    ep2017.nodes_ttl[node_id] = ttl
    sleep(0.2)
...
def clean_nodes():
    """
    """
    for node in copy(ep2017.nodes):
        if time() > ep2017.nodes_ttl[node]:
            print('cleaning node', node)
            ep2017.nodes.remove(node)
            ep2017.nodes_ttl.pop(node)
            sleep(0.2)
    print('{} active nodes'.format(len(ep2017.nodes)))
...
def get_img_url_for_node(node_id):
    """
    """
    if ep2017.nodes[get_index_from_md5(WIN_URL, ep2017.nodes)] == node_id:
        img_url = WIN_URL
        update_winners(node_id)
    else:
        img_index = get_index_from_md5(node_id, IMG_URLS)
...
def get_live_stats():
    ...
    """
    try:
        winner_node = ep2017.nodes[get_index_from_md5(WIN_URL, ep2017.nodes)]
    except ZeroDivisionError:
        winner_node = 'None'
    return len(ep2017.nodes), winner_node
...
if __name__ == "__main__":
    """
    Modulo implementation uses calculated indexes so it needs
        - a list to locate the image URL of a given node id
        - a dict to store nodes time to live (TTL)
    """
